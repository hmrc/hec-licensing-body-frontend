...
@Singleton
class TaxCheckCodeValidController @Inject() (
  ...
  val taxCheckMatch: Action[AnyContent] = sessionDataAction { implicit request =>
    Ok(
      s"Session is ${request.sessionData} back Url ::${journeyService.previous(routes.TaxCheckCodeValidController.taxCheckMatch())}"
    )
  }
